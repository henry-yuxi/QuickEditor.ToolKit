...
namespace BuiltinViewer
{
	public class BuiltinGUISkinViewer : EditorWindow
	{
		...
		{
			//Debug.Log(m_sampleTitleStyle.name);
			EditorGUI.LabelField(position, new GUIContent(m_selectedStyle == null ? "---" : m_selectedStyle.name), new GUIContent(m_selectedStyle == null ? "---" : m_selectedStyle.name));
			position.y += position.height;
